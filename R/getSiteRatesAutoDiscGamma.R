...
getSiteRatesAutoDiscGamma <- function(n_nodes, n_tips, n_states, edge_len, edge_mat, parents, root_node, X, Q, M, root_type, beta, k, n.cores){
    ...
    if( nsites == 4){
        ...
        for( cat in 1:k ){
            ...
            lik_unit <- N_unit ## Start the loop.
            ## For site 2:
            lik_unit <- sapply(1:k, function(i) getIntUnit(gamma.lik=gamma.lik, M=M, i=i, k=k, n=2, left_unit=lik_unit, keep = keep))
            ## For site 1:
            marg[cat, nsites] <- getFirstUnit(gamma.lik=gamma.lik, k=k, second_unit=lik_unit)
        }
        ...
        for( cat in 1:k ){
            ...
            lik_unit <- sapply(1:k, function(i) log(M[i,cat]) + gamma.lik[[3]][cat] + lik_unit[cat])
    ...
    if( nsites == 2){
        ...
        ## Marginal for the last site.
        for( cat in 1:k ){
            lik_unit <- sapply(1:k, function(i) log(M[i,cat]) + gamma.lik[[nsites]][cat] )
            ## For site 1:
            marg[cat, nsites] <- getFirstUnit(gamma.lik=gamma.lik, k=k, second_unit=lik_unit)
        }
