...
##' @param bounds 
##' @param state.space can be "max.div", "site.div" or a numeric. The size of the state space is equal to the largest observed or equal to the per site state diversity plus the number.
...
##' @export
##' @author daniel
fitMLGammaSpanSpace <- function(data, phy, root.type = "madfitz", ncat = 4, bounds = c(0,100), state.space = "max.div", opts = NULL, init = NULL, verbose = TRUE, n.cores = 1){
    ...
    if( is.character(state.space) ){
        ...
        if( !length( state.space ) == 1 ) stop("state.space needs to be 'max.div', 'obs.div' or a single number.")
